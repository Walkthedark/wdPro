				...
				if (params.goodsData[int].goods_id == this.getpromotion.product[gt].promotion_goods) {
					...
					if(Num != -1){
						...
						if (this.getpromotion.product[gt].promotion_rule == 4) {//这个是赠品
							...
							//判断是赠品内容
//							var gDataPro =  promotion_detailH[Num].product;//获取满赠的商品
//							var pIdFlag = {};//满足条件返回是
//							pIdFlag.pId = params.goodsData[int].goods_id;
//							console.log(pIdFlag);
		...
		for (var pr = 0; pr < this.getpromotion.group.length; pr++) {// 循环的是促销商品的活动
			...
			if (temporary_list.length <= 0) {// 没找到
				...
				if (promotion_typeL == 1) {// 按数量
					...
					var Num = this.getNumMoney(promotion_detailL, numAndMoney, 1);//判断是否满足组合促销
		 ...
		 */
		var res = {};
		if (this.getpromotion.order.promotion_rule != null && this.getpromotion.order.promotion_rule != "") {// 如果为空则表示该用户没有订单优惠
			var t = [];
	...
	common.getPromotionActiv = function(clinet_id) {
		// 获取促销活动
		var params = {};
		params.client_id = localStorage.getItem("company_id");
