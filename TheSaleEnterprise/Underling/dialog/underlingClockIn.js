define(function(require) {
	...
	Model.prototype.getMonthData = function() {
		var self = this;
		var cDate = this.comp("cDate");
		var time = $("[xid=dateSpan]").text();
		var type = cDate.getValue("type");
		if (!asCore.isNull(type)) {
			type = 1;
		}
		var sendUrl = "/index/mobile/xskqcxybb";
		var pData = this.comp("pData");
		var data = {};
		data.yue = time;
		data.zjxs = type;
		asCore.xskqcxybb(sendUrl, data, function(data) {
	...
	Model.prototype.getDayData = function() {
		var cDate = this.comp("cDate");
		var time = $("[xid=dateSpan2]").text();
		var type = cDate.getValue("type");
		if (!asCore.isNull(type)) {
			type = 1;
		}
		var self = this;
		var sendUrl = "/index/mobile/xskqcxrbb";
		var dData = this.comp("dData");
		var data = {};
		data.yue = time;
		data.zjxs = type;
		asCore.xskqcxrbb(sendUrl, data, function(data) {
	...
	Model.prototype.button8Click = function(event) {
		$("[xid=dateSpan2]").text(this.GetDateStr2($("[xid=dateSpan2]").text(), -1));
		this.getDayData();
	};
