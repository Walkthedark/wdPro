define(function(require) {
		...
		geolocation.getCurrentPosition(function(r) {
			if (this.getStatus() == BMAP_STATUS_SUCCESS) {
				...
				geoc.getLocation(pt, function(rs) {
					...
					// console.log(address);
					$("[xid=branchesAddress]").text(address);
				});
			} else {
	...
	Model.prototype.eventBind = function() {
		var that = this;
		$("#upload3").click();
		$("#upload3").unbind().change(function() {
	...
	Model.prototype.compress = function(res) {
		...
		if (length < 6) {
			...
			img.onload = function() {
				...
				var Insert = document.getElementById("netCollectImg1");
				var div6 = document.getElementById("netCollectImg");
	...
	Model.prototype.button4Click = function(event) {
		var self = this;
		var sendUrl = "/index/mobile/netsjcj";
		...
		asCore.netsjcj(sendUrl, data, function(data) {
			if (data.success) {
