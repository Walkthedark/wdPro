define(function(require) {
	...
	Model.prototype.init = function() {
		$("#customerCollectImg1").show();
		this.eventBind();
	};
	Model.prototype.eventBind = function() {
		var that = this;
		$("#upload1").click();
		$("#upload1").unbind().change(function() {
	...
	Model.prototype.compress = function(res) {
		...
		if (length < 6) {
			...
			img.onload = function() {
				...
				var Insert = document.getElementById("customerCollectImg1");
				var div6 = document.getElementById("customerCollectImg");
	...
	Model.prototype.button8Click = function(event) {
		...
		if (address) {
			this.comp("CData").setValue("text", address);
		} else {
			justep.Util.hint("请先获取地理位置信息");
		}
	};
	...
	Model.prototype.button4Click = function(event) {
		var self = this;
		var sendUrl = "/index/mobile/jxssjcj";
		var address = this.comp("CData").getValue("text");
		...
		asCore.jxssjcj(sendUrl, data, function(data) {
			if (data.success) {
