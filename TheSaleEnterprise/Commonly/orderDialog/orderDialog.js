define(function(require) {
	...
	Model.prototype.addCartClick = function(event) {
		...
		params.specification = specification1_name + ":" + specification1_value;
	...
	Model.prototype.button7Click = function(event){
		...
		GoodsInfodata.each(function(p) {
			...
			if(p.row.val("goods_num") != null && p.row.val("goods_num") != 0){
				params.uuid = p.row.val("uuid");
				params.goods_id = p.row.val("goods_id");
				params.specification_id = p.row.val("specification_id");
				params.goods_img = $(self.getElementByXid("image8"))[0].src;
				params.goods_name = p.row.val("goods_name");
				params.goods_num = p.row.val("goods_num");
				params.price = p.row.val("price");
				params.unit_id = p.row.val("unit_id");
				params.specification = p.row.val("specification1_name") + ":" + p.row.val("specification1_value");
				console.log(params);
			}
		});
	};
