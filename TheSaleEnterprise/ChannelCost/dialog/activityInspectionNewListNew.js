define(function(require) {
	...
	Model.prototype.map = function(event) {
		...
		geolocation.getCurrentPosition(function(r) {
			if (this.getStatus() == BMAP_STATUS_SUCCESS) {
				...
				cDate.setValue("lng", r.point.lng);
				cDate.setValue("lat", r.point.lat);
	...
	Model.prototype.getTypeList = function() {
		...
		var sendUrl = "/index/mobile/newschdjctype";
		var data = {
			id : this.mxId
		};
		asCore.newschdjctype(sendUrl, data, function(data) {
			CData.loadData(data.data);
		});
	}
	...
	Model.prototype.init = function() {
		$("#activityInspectionNewListImg1").show();
		this.eventBind();
	};
	Model.prototype.eventBind = function() {
		var that = this;
		$("#uploadactivityInspectionNewList").click();
		$("#uploadactivityInspectionNewList").unbind().change(function() {
	...
	Model.prototype.compress = function(res) {
		...
		if (length < 6) {
			...
			img.onload = function() {
				...
				var Insert = document.getElementById("activityInspectionNewListImg1");
				var div6 = document.getElementById("activityInspectionNewListImg");
	...
	Model.prototype.upload = function(image_data, id) {
		...
		justep.Baas.sendRequest({
			...
			success : function(data) {
				$('#uploadactivityInspectionNewList').val('');
				if (data.error == "0") {
	...
	Model.prototype.button4Click = function(event) {
		var cData = this.comp("cData");
		var qdlx = cData.getValue("qdlx");
		var qddz = cData.getValue("qddz");
		var qdms = cData.getValue("qdms");
		var lat = cData.getValue("lat");
		var lng = cData.getValue("lng");
		var self = this;
		var sendUrl = "/index/mobile/newschdjc";
		var qdzp = "";
		...
		data.lng = lng;
		data.photo = qdzp;
		asCore.newschdjc(sendUrl, data, function(data) {
			if (data.success == 1) {
