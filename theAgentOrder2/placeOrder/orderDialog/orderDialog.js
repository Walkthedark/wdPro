define(function(require) {
	...
	Model.prototype.li1Click = function(event) {
		...
		if (res.code === 200) {
			$(this.getElementByXid("span301")).html("1");
...
//		this.close();
	...
	Model.prototype.addZBtnClick = function(event) {
		...
		var goods_sn = row.val("goods_sn");// 获取商品的编码
		...
		if (goods_id != null || goods_id != "") {
			asCore.ListloadGoodsOrderInfo(p);
...
//			this.close();
	...
	Model.prototype.button4Click = function(event) {
		...
		var params = {};
		var quantity = parseInt($(this.getElementByXid("span301")).html());
			...
			if (classNameData2.getCount() > 0) {// 有第二级别的
				if (this.classValue2 != "") {
					if (this.classValueList2.length > 0) {
						...
						var u_id = unitData.getRowID();
						params.unit_id = u_id;// 添加单位id////不应该在这取单位的值
					...
					//	params.price = this.classValueList2[0].price;
						params.price = unitData.getValueByID("price", u_id)
...
//					params.price = this.classValueList3[0].price;
	...
	Model.prototype.li6Click = function(event) {
		...
		// 查找二级的
		var res = asCore.search(this.classValueList, "specification2_value", name);// 获取所有选择一级之后的二级的目录//这里是二级的数据并且会三级的数据也存在这里
		//
...
//		this.comp("list6").refresh(false);
	...
	Model.prototype.li7Click = function(event) {
		...
		classValue2Data.each(function(param) {
			...
			if (nameVList.length <= 0) {
				param.row.val("flag", 0);//没有这些数据，可以选择
			} else {
				param.row.val("flag", 1);//有这些数据，可以选择
			};
		});
	};
	// 第三个分类
	...
	Model.prototype.button19Click = function(event) {
		var val = parseInt($(this.getElementByXid("span301")).html());
		$(this.getElementByXid("span301")).html(val += 1);
	};
	// 详情减
	...
	Model.prototype.button20Click = function(event){
		justep.Shell.showPage("cartList",{flag:1});
	};
