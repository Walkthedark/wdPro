define(function(require) {
	...
	Model.prototype.getAddressInfo = function(event) {
		...
		if (res.code === 200) {
			...
			localStorage.setItem("address_id", res.data.client_address_id);
			var p = [];
			p.push(res.data);
	...
	// 将所有商品详情选择的商品加进去
	Model.prototype.ListloadGoodsOrderInfo = function(p) {
		// 获取
		var params = {};
		// 添加客户的ID
		params.clinet_id = this.cClinet_id;
	...
	Model.prototype.addGoodsDataList = function(uuid, flag) {
		...
		goodsDataM.loadData(res.returnDoodsData, true, null, 1, true);
		localStorage.setItem("cartList",JSON.stringify(res.returnDoodsData));
	};
	// 加
...
//		console.log(params);
	...
	Model.prototype.giftListloadGoodsOrderInfo = function(param) {
		...
		data2.loadData(param, false, null, 1, true);
	};
	// 赠品加
	...
	Model.prototype.modelParamsReceive = function(event){
		...
		//获取默认地址
		this.adressDetail(event);
		//获取塞入值
		//ListloadGoodsOrderInfo
		var json = localStorage.getItem("cartList");
		var jsonData = JSON.parse(json);
		this.goodsDataListChoose = jsonData;
		this.ListloadGoodsOrderInfo(jsonData);
		var giftListJson = localStorage.getItem("giftList");
		var giftJsonData = JSON.parse(giftListJson);
		for (var int = 0; int < giftJsonData.length; int++) {
			giftJsonData[int].num = giftJsonData[int].goods_num;
		}
		this.giftGoodsList  = giftJsonData;
		this.giftListloadGoodsOrderInfo(giftJsonData);
		
	};
	//获取地址
	Model.prototype.adressDetail = function(event){
		var address_id =  localStorage.getItem("address_id");
		if(address_id !=null ||  address_id != ""){//判断地址数据不能为空
		var params = {};
			params.id = address_id;
		var res = asCore.setGetFalse(inFace.adressDetail,params);
		if(res.code === 200){
			var addressData = this.comp("addressData");
			if (res.data.client_address_id == null) {
				addressData.clear();
