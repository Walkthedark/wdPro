define(function(require) {
	...
	Model.prototype.button1Click = function(event) {
		...
		var dateName = common.nextDate(DateValue);
		this.comp("input2").val(dateName);
		// 重新复制
		this.getRefassh(dateName);
	};
	// 上一天
	Model.prototype.button2Click = function(event) {
		var DateValue = this.comp("input2").val();
		var dateName = common.Previousday(DateValue);
		this.comp("input2").val(dateName);
		// 重新搜索
		this.getRefassh(dateName);
	};
	// refash
	Model.prototype.getRefassh = function(dateName) {
		var params = {};
		params.date = dateName ;
		var res = asCore.setGetFalse(interFace.cc_mine_report, params);
		if (res.code === 200) {
			this.comp("myreData").loadData(res.data, false);
			this.comp("myreData").refreshData();
