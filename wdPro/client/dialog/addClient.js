define(function(require) {
	...
	Model.prototype.div77Click = function(event) {
		...
		if (asCore.regular(nameInpV)) {
			justep.Util.hint("客户名不能为空",{'type':'danger'});
			return;
		}
		if (asCore.regular(client_id)) {
			justep.Util.hint("客户编码不能为空",{'type':'danger'});
			return;
		}
		// 获取客户分类IDcategory
		...
		// 验证客户分类
		if (this.category == "") {
			justep.Util.hint("客户分类不能为空",{'type':'danger'});
			return;
		}
		;
		var level_id = dataJson.getValue("level_id");
		if (level_id == null) {
			justep.Util.hint("客户等级不能为空",{'type':'danger'});
			return;
		};
		...
		if (this.region_ID_Type == "") {
			justep.Util.hint("销售区域不能为空",{'type':'danger'});
			return;
		}
		;
		// 获取销售区域
