define(function(require) {
	...
	Model.prototype.div77Click = function(event) {
		...
		;
		var level_id = clientData.getValue("level_id");
		...
		params.sales_manager = this.busmanager;
		// 业务模式business_type_Inp
		params.business_type = this.business_Type;
		// 客户来源
		params.source = clientData.getValue("source");
		// 成立时间
		params.establish_time = clientData.getValue("establish_time");
		// 服务满意度
		params.service_rate = clientData.getValue("service_rate");
		// 交易印象
		params.deal_impression = clientData.getValue("deal_impression");
		// 企业性质
		params.enterprise_nature = clientData.getValue("enterprise_nature");
		// 年营业
		params.year_turnover = clientData.getValue("year_turnover");
		// 企业模式
		params.enterprise_scale = clientData.getValue("enterprise_scale");
		// 获取开户名称
		...
		// 获取企业
		params.trade_id = clientData.getValue("trade_id");
		// 这里是1053的新增值
...
//		}
	...
	Model.prototype.saveClient = function(params) {
		var res = asCore.setPostFalse(interFace.editClientInfo, params);
		if (res.code === 200) {
			 justep.Util.hint("客户修改成功");
			 history.back();	
		} else {
			justep.Util.hint("客户修改失败",{'type':'danger'});
		}
	};
	// 清空数据
	...
	Model.prototype.modelLoad = function(event) {
		...
		if (res.code === 200) {
			...
			var p =[];
			p.push(res.data);
			clientData.loadData(p,false);
			clientData.refreshData();
			this.category = res.data.category;// 客户分类
			this.region_ID_Type = res.data.region_id;// 销售区域
			this.business_Type = res.data.business_Type;// 业务模式
			//插入jsonData值dataJson//10001
			var dataJson = this.comp("dataJson");
			dataJson.setValueByID("bank_account", res.data.bill.bank_account,10001);
			dataJson.setValueByID("bank_name", res.data.bill.bank_name,10001);
			dataJson.setValueByID("bank", res.data.bill.bank,10001);
			dataJson.setValueByID("invoice", res.data.bill.invoice,10001);
			dataJson.setValueByID("invoice_address", res.data.bill.invoice_address,10001);
			dataJson.setValueByID("invoice_mobile", res.data.bill.invoice_mobile,10001);
			dataJson.setValueByID("remark", res.data.bill.remark,10001);
			dataJson.setValueByID("tax_num", res.data.bill.tax_num,10001);
			//插入
				var sales_manager_id =[];
				var sales_manager_name ="";
				for (var int = 0; int <res.data.sales_manager.length; int++) {
					sales_manager_id.push(res.data.sales_manager[int].staff_id);
					sales_manager_name+=res.data.sales_manager[int].staff_name+","
				}
				this.comp("input2").val(sales_manager_name);
				this.busmanager = sales_manager_id;
//			var
		} else {
			justep.Util.hint("客户详情加载失败",{'type':'danger'});
		}
	...
	Model.prototype.getClinet_Type = function(params) {
		...
		var clientData = this.comp("clientData");
		clientData.setValueByID("category",  params.ID, this.id);
		clientData.setValueByID("category_name", params.Name, this.id);
	...
	Model.prototype.getBusiness_type = function(params) {
		...
		var clientData = this.comp("clientData");
		clientData.setValueByID("business_Type",  params.Name, this.id);
	};
	// 打开销售区域
	...
	Model.prototype.getRegion_ID = function(params) {
		...
		var clientData = this.comp("clientData");
		clientData.setValueByID("region_id",  params.ID, this.id);
		clientData.setValueByID("region_name", params.Name, this.id);
	};
	// /从这里开始往data里面set值
	// 客户等级1:代表客户等级
	...
	Model.prototype.getCommon_Radio = function(params) {
		...
		if (params.flag == 1) {
			...
			clientData.setValueByID("level_id",  params.ID, this.id);
	...
	Model.prototype.div77Dblclick = function(event){
	...
	Model.prototype.button3Click = function(event){
		...
		justep.Shell.showPage("busManagerClient",{data:p});
	};
	//获取业务经理
	Model.prototype.getbusManagerClient = function(params){
		this.comp("input2").val(params.Name);//负值
		this.busmanager = params.ID;
	};
	return Model;
});