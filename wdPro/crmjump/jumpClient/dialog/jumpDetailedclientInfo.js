define(function(require) {
	...
	Model.prototype.saveClient = function(params) {
		...
		var res = asCore.setPostFalse(interFace.saveClient, params);
		if (res.code === 200) {
			this.clearInput();
			// successClient
			justep.Shell.showPage("successClient", {
				order_id : this.client_id_
			});
			// justep.Util.hint("客户新增成功");
		} else {
			justep.Util.hint("客户新增失败");
		}
	};
	// 清空数据
...
//		params.id = this.params.data.id;
		params.id = this.getContext().getRequestParameter("id");
		this.id = params.id;
