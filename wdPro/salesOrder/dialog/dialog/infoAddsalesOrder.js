define(function(require) {
	...
	Model.prototype.div77Click = function(event) {
		// 获取值
		var params = {};
		params.documentary_id = this.documentary_id;
		...
		// <column name="camera" type="String" xid="xid26" label="摄像"></column>
		...
		par.MC = MC;
		// <column name="dessert" type="String" xid="xid30" label="甜品"></column>
		var dessert = this.comp("input32").val();
		par.dessert = dessert;
		// <column name="acting" type="String" xid="xid31" label="演艺"></column>
		...
		// <column name="TheCar" type="String" xid="xid33" label="婚车"></column>
		...
		par.ASuit = ASuit;
		// <column name="dress" type="String" xid="xid36" label="礼服"></column>
		...
		// <column name="joyful" type="String" xid="xid38" label="喜糖"></column>
		...
		// <column name="bestMan" type="String" xid="xid43" label="伴郎"></column>
	...
	Model.prototype.setAdd = function(params) {
		//edit
		var res = asCore.setPostFalse(interFace.editDocumentary, params);
		if (res.code === 200) {
			justep.Util.hint("修改跟单成功");
		} else {
			justep.Util.hint("修改跟单失败");
		}
	};
	// 打开关联客户
	...
	Model.prototype.modelParamsReceive = function(event) {
		...
		if (res.code === 200) {
			// set title
			this.comp("input1").val(res.data.title);
			// set 客户名称
			
			this.client_id = res.data.client_id;
			// set stage
			this.comp("input21").val(res.data.stage);
			this.stage_id = res.data.stage_id;
			// set money
			this.comp("input2").val(res.data.money);
			// set remark
			this.comp("input3").val(res.data.remark);
			//set link
			this.linkM  = res.data.staff_id;
			var p =[];
			//
			var goodsData = this.comp("goodsData");
			goodsData.loadData(res.data.goods, false);
			for (var int = 0; int < res.data.goods.length; int++) {
				p.push(res.data.goods[int].goods_id);
			};
			this.linkM = p;
			if(res.data.client_id != null){
				this.comp("input4").val(res.data.client.client_name);
			};
			...
			if(remark != null && remark !=""){
					var r =  JSON.parse(remark);
					this.comp("contentData").loadData(r, false);
					this.comp("contentData").refreshData();
			}
			
		} else {
			justep.Util.hint("跟单详情获取失败",{"type":'danger'});
		}
	};
	return Model;
});