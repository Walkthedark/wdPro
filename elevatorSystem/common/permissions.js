define(function(require) {
	var $ = require("jquery");
	var server = require("$UI/elevatorSystem/interfaces/interfaces");
	var justep = require("$UI/system/lib/justep");
	require("css!$UI/elevatorSystem/InstallContact/js/layer/skin/default/layer").load();
	require("$UI/elevatorSystem/InstallContact/js/PerfectLoad");
	require("$UI/elevatorSystem/InstallContact/js/layer/layer");
	var self = this;
	var fn = {
		getPermissions : function(access) {
			...
			$.ajax({
				method : "GET",
				url : server.path + 'BaseData/GNAccess',
				cache : false,
				async : false,
				data : {
		...
		getWeiXinImage : function(mediaId,sqlName,id,flag) {
			...
			$.ajax({
				method : "GET",
				url : server.path + 'WeiXinQiYe/downloadImage',
				cache : false,
				async : false,
				data : {
					token : localStorage.getItem('token'),
		...
		getCommonUp : function(url,name,size,sqlName,id) {
			...
			$.ajax({
				method : "POST",
				url : server.path + 'File/add?token='+localStorage.getItem('token'),
				cache : false,
				async : false,
				data : {
					url:url,//图片路径
		...
		getOnlyImgUp : function(id,type) {
			...
			$.bootstrapLoading.start({
			 ...
			 $.ajax({
				"url" : server.path + 'UploadFile/search',
				"type" : 'GET',
				cache : false,
				"async" : false,
				"dataType" : 'json',
		...
		//从公共的的接口获取照片
		getCommonImg : function(id,sqlName) {
			var res;
			$.bootstrapLoading.start({
				loadingTips : "正在获取中..."
			});
			 $.ajax({
				"url" : server.path + 'File/select',
				"type" : 'GET',
				cache : false,
				"async" : false,
				"dataType" : 'json',
			...
			if (data != "" && data != "undefined" && data != "null" && typeof (data) != "undefined") {
				for (var i = 0; i < data.length; i++) {
					...
					//	newName=
							...
							newUrl = this.getImageUrl("$UI/elevatorSystem/img/Office/Excel.png");
						else if (extension.toUpperCase() == "DOC" || extension.toUpperCase() == "DOCX")
							newUrl = this.getImageUrl("$UI/elevatorSystem/img/Office/Word.png");
						else if (extension.toUpperCase() == "POWERPOINT")
							newUrl = this.getImageUrl("$UI/elevatorSystem/img/Office/PowerPoint.png");
						else
							newUrl = this.getImageUrl("$UI/elevatorSystem/img/Office/UnKnow.png");
					} else {
						if (extension.toUpperCase() == "PNG" || extension.toUpperCase() == "JPG" || extension.toUpperCase() == "JIF" || extension.toUpperCase() == "BMP"){
							...
							newUrl = fn.getImageUrl("$UI/elevatorSystem/img/Office/Excel.png");
						else if (extension.toUpperCase() == "DOC" || extension.toUpperCase() == "DOCX")
							newUrl = fn.getImageUrl("$UI/elevatorSystem/img/Office/Word.png");
						else if (extension.toUpperCase() == "POWERPOINT")
							newUrl = fn.getImageUrl("$UI/elevatorSystem/img/Office/PowerPoint.png");
						else if (extension.toUpperCase() == "TXT")
							newUrl = fn.getImageUrl("$UI/elevatorSystem/img/Office/filetxt.png");
						else
							newUrl = fn.getImageUrl("$UI/elevatorSystem/img/Office/UnKnow.png");
		...
		upOnlyImg : function(url,name,size,sqlName,id) {
			...
			$.ajax({
				method : "POST",
				url : server.path + 'UploadFile/addApi?token='+localStorage.getItem('token'),
				cache : false,
				async : false,
				data : {
					PID:id,//图片路径
